...
//┅┅┅┅〔 Fitur Tambahan Dari Pebri 〕┅┅┅┅//
           ...
           break
	       case 'nulis2':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/canvas/nulis2?text=${body.slice(8)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break
	       case 'nuliskiri':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`https://hardianto-chan.herokuapp.com/api/nuliskiri?text=${body.slice(11)}&apikey=${hardiapi}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break
	       case 'narutobanner':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/manga-naruto?teks=${body.slice(14)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break	       
	       case 'fireteks':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/flaming-fire?teks=${body.slice(10)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break	       
	       case 'goldteks':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/metalic-gold?teks=${body.slice(10)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break
	       case 'skyteks':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/shadow-sky?teks=${body.slice(9)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break
	       case 'cupteks':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
	       peb = await getBuffer(`http://hadi-api.herokuapp.com/api/photoxy/teks-cup?teks=${body.slice(9)}`)
	       fdhl.sendMessage(from, peb, image, {quoted: mek})
	       break
	       case 'loveteks':
	       if (args.lenght < 1 ) return reply(`TeksNyaLol`)
